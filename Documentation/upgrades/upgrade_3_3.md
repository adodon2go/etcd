...
```diff
import "github.com/adodon2go/etcd/etcdserver"
...
```go
import "github.com/adodon2go/etcd/embed"
...
Previously, `[endpoint]:[client-port]/health` returned manually marshaled JSON value. 3.3 now defines [`etcdhttp.Health`](https://godoc.org/github.com/adodon2go/etcd/etcdserver/api/etcdhttp#Health) struct.
...
```go
import "github.com/adodon2go/etcd/embed"
import "github.com/adodon2go/etcd/etcdserver/api/v3rpc/rpctypes"
...
```go
import "github.com/adodon2go/etcd/clientv3"
...
**If not specified, client-side send limit defaults to 2 MiB (1.5 MiB + gRPC overhead bytes) and receive limit to `math.MaxInt32`**. Please see [clientv3 godoc](https://godoc.org/github.com/adodon2go/etcd/clientv3#Config) for more detail.
...
`clientv3` has deprecated `golang.org/x/net/context`. If a project vendors `golang.org/x/net/context` in other code (e.g. etcd generated protocol buffer code) and imports `github.com/adodon2go/etcd/clientv3`, it requires Go 1.9+ to compile.
